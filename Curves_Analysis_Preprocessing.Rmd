...
```{r}
myfiles = list.files(path = here::here("Participant_Data/Session_1"), pattern = c("Curves.Startingpos"), full.names = TRUE)
...
```{r}
# participant 9
myfiles = list.files(path = here::here("Participant_Data/Session_2"), pattern = c("Startingpos_9"), full.names = TRUE)
dat_9 <- ldply(myfiles, read_csv) %>%
  dplyr::mutate(pNum = 9)

# participant 10
myfiles = list.files(path = here::here("Participant_Data/Session_2"), pattern = c("Startingpos_10"), full.names = TRUE)
dat_10 <- ldply(myfiles, read_csv) %>%
  dplyr::mutate(pNum = 10)

# participant 11
myfiles = list.files(path = here::here("Participant_Data/Session_2"), pattern = c("Startingpos_11"), full.names = TRUE)
dat_11 <- ldply(myfiles, read_csv) %>%
  dplyr::mutate(pNum = 11)

# participant 12
myfiles = list.files(path = here::here("Participant_Data/Session_2"), pattern = c("Startingpos_12"), full.names = TRUE)
...
used to calculate percentage of trials per condition

```{r}
total.trials <- dat_new %>%
...
write.csv(total.trials, file = here::here("Participant_Data/total_trials.csv"))
```
...
```{r}
RUN RADII_MIDLINE FILE from this chunk.
```
...
# data saving
write.csv(overall.dat, file = here::here("Participant_Data/overall_dat.csv"))
write.csv(onset.timecourse, file = here::here("Participant_Data/onset_timecourse.csv"))
```