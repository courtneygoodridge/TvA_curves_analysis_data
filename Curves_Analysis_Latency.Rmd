...
```{r}
"load coordinates"
latency.1000 <- read.csv(here::here("TvA_curves_analysis_data/Midline_Data/Curves.Startingpos_radius_1500_latency.csv")) %>%
...
latency.1000.8$visangle_rads <- iterate_bias(df = latency.1000.8)

latency.1000.8 <- latency.1000.8 %>%
  dplyr::mutate(visangle_rads = abs(visangle_rads)) %>%
  dplyr::mutate(visangle = visangle_rads * 180/pi)

ggplot(as.data.frame(latency.list[[1]]) %>%
         dplyr::filter(timestamp < 2), mapping = aes(x = timestamp, y = visangle)) +
  geom_path()

ggplot(as.data.frame(latency.list[[1]]) %>%
         dplyr::filter(timestamp < 2), mapping = aes(x = timestamp, y = steeringbias)) +
  geom_path()
...
for (i in 1:9){
  ...
  for (a in c(.9)){
    ...
    latencies_tmp <- latencies_tmp %>%
      dplyr::mutate(visangle_accum = cumsum(visangle)) %>%
      dplyr::filter(visangle_accum > a) %>%
...
ggplot(df_accum, mapping = aes(x = radius, y = timestamp, col = as.factor(startingpos))) +
  geom_point() 

ggplot(df_accum, mapping = aes(x = radius, y = steeringbias, col = as.factor(startingpos))) +
  geom_point()
```
...
ggplot(df_thresh, mapping = aes(x = radius, y = timestamp, col = as.factor(startingpos))) +
  geom_point() 

ggplot(df_thresh, mapping = aes(x = radius, y = steeringbias, col = as.factor(startingpos))) +
  geom_line() +
  ylim(0, .07)
```


## Threshold predictions with 150 ms motor latency across differing thresholds
...
for (i in 1:9){
  ...
  for (a in c(seq(0.01, 0.04, by = 0.01))){
...
threshold.labs <- c("0.01" = "Threshold: 0.01 m", "0.02" = "Threshold: 0.02 m", "0.03" = "Threshold: 0.03 m", "0.04" = "Threshold: 0.04 m")
