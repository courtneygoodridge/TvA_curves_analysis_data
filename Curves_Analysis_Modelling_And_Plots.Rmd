...
```{r}
total.trials <- read.csv(here::here("TvA_curves_analysis_data/Participant_Data/total_trials.csv")) %>%
  dplyr::select(total)
...
ggplot() +
  ...
  geom_curve(aes(x = 0, y = 0, xend = 0.0471, yend = 10), arrow.fill = "black", size = 0.3, curvature = 0) +
...
```{r}
acc.steering <- read.csv(here::here("TvA_curves_analysis_data/Prediction_Data/accumulator.steering.preds.radii.steering.csv")) 
