...
### Error accumulation when steering toward curves - Modelling and plots 
...
This script produces figures for the manuscript and multilevel models for reaction time, lateral position error, and steering magnitude.
...
Removing participant 10 because they did not have a UK driving license at the time of testing.

```{r}
overall.dat <- read.csv(here::here("TvA_curves_analysis_data/Participant_Data/overall_dat.csv")) %>%
...
ggplot() +
  ...
  ggsave(here::here("TvA_curves_analysis_data/Plots/Fig1.tiff"), plot = last_plot(), width = 7, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 2: camera counter rotation in Goodridge et al (2022)

```{r}
trajectory <- read.csv(here::here("TvA_curves_analysis_data/Midline_Data/Exp_Design_Data/trajectory.csv"))
experiment_2_coordinates <- read.csv(here::here("TvA_curves_analysis_data/Midline_Data/Exp_Design_Data/experiment_2_coordinates.csv"))
...
g <- arrangeGrob(no.cam.rotation, cam.rotation, nrow = 1)
  ggsave(here::here("TvA_curves_analysis_data/Plots/Fig2.tiff"), g,  width = 14, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 3 - visual angle and paradigm example
...
ggplot() +
  ...
  geom_point(mapping = aes(x = 0.0471, y = 10), shape = 8) +
  geom_curve(aes(x = 0, y = 0, xend = 0.0471, yend = 10), linetype = "dashed", arrow.fill = "black", size = 0.3, curvature = 0) +
  geom_curve(aes(x = 0, y = 0, xend = 0, yend = 12), arrow.fill = "black", size = 0.3, curvature = 0, arrow = arrow(type = "closed", length = unit(0.3,"cm"))) +
  annotate("text", x = 0.37, y = 40, size = 3, label = "2000 m") +
  annotate("text", x = 0.37, y = 32.5, size = 3, label = "1500 m") +
  annotate("text", x = 0.37, y = 26.5, size = 3, label = "1000 m") +
  ylim(0, 40) +
  xlim(0, 0.40) +
  ylab("z (m)") +
  xlab("x (m)") +
...
ggsave(here::here("TvA_curves_analysis_data/Plots/Fig3.tiff"), last_plot(),  width = 7, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 4: Reaction time predictions and data

```{r}
"Load data"
df_accum <- read.csv(here::here("TvA_curves_analysis_data/Prediction_Data/df_accum.csv"))
df_thresh <- read.csv(here::here("TvA_curves_analysis_data/Prediction_Data/df_thresh.csv"))

"REACTION TIME PREDICTIONS"
accum.plot.rt <- ggplot(df_accum, mapping = aes(x = radius.x, y = timestamp, linetype = as.factor(startingpos))) +
  geom_line() +
  ylim(0.2, .65) +
...
## Figure 5: Lateral position error predictions

```{r}
"LATERAL POSITION ERROR PREDICTIONS"
accum.plot.lpe <- ggplot(df_accum, mapping = aes(x = radius.x, y = steeringbias, linetype = as.factor(startingpos))) +
  geom_line() +
  scale_x_continuous(breaks = c(1000, 1500, 2000), limits = c(900, 2100)) +
  ylim(0, 0.07) +
  xlab("Radius (m)") +
...
thresh.plot.lpe <- ggplot(df_thresh, mapping = aes(x = radius.x, y = steeringbias, linetype = as.factor(startingpos))) +
  geom_line() +
  scale_x_continuous(breaks = c(1000, 1500, 2000), limits = c(900, 2100)) +
  ylim(0, .15) +
  xlab("Radius (m)") +
...
"load/create data"
df_accum <- df_accum %>%
  dplyr::mutate(steering_rate = steeringbias * 15)
  
thresh.steering <- read.csv(here::here("TvA_curves_analysis_data/Prediction_Data/threshold.steering.preds.radii.steering.csv")) 

"need to adjust steering to put make lines more visible for plotting"
...
acc.plot.sr <- ggplot(df_accum, mapping = aes(x = radius.x, y = steering_rate, linetype = as.factor(startingpos))) +
  geom_line() +
  scale_x_continuous(breaks = c(1000, 1500, 2000), limits = c(900, 2100)) +
  ylim(0, 0.85) +
  xlab("Radius (m)") +
  ...
  scale_linetype_manual(name = "Starting position (m)", labels = c("0", "4", "8"),  values = c("dotted", "dashed", "solid"), guide = guide_legend(reverse = TRUE)) +
  theme_plot +
  theme(legend.position = c(.60, .85), legend.key.height = unit(0, 'cm'), legend.key.width = unit(0.5, 'cm'), legend.key = element_blank(), strip.text.x = element_blank(), panel.spacing = unit(2, "lines"), legend.spacing.y = unit(0.1, "cm"), legend.margin = margin(0.05,0.05,0,0, unit = "cm"), legend.key.size = unit(0, 'cm'), plot.title = element_text(size = 12, face = "bold"), axis.text.x = element_text(size = 11), axis.title.x = element_text(size = 11), axis.title.y = element_text(size = 11), title = element_text(size = 7), legend.title = element_text(size = 10), legend.text = element_text(size = 10), axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.line.y = element_line(arrow = grid::arrow(length = unit(0.4, "cm"), ends = "last")))

"Threshold framework predictions plot"
thresh.plot.sr <- ggplot(thresh.steering %>%
...
ggsave(here::here("TvA_curves_analysis_data/Plots/Fig6.tiff"), g,  width = 10, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```
...
ggsave(here::here("TvA_curves_analysis_data/Plots/Fig8.tiff"), plot = last_plot(), width = 10, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```
...
"1000 radius curve"
radius.1000.plot <- ggplot() +
  geom_path(data = midline %>%
              dplyr::filter(radius == 1000), mapping = aes(x = x * -1, y = z), size = 1) +
  geom_path(data = onset.timecourse %>%
              dplyr::filter(pNum != 10, radius == 1000) %>%
  ...
  scale_fill_manual(name = "Starting \nposition (m)", values = c("black", "white", "darkgrey"), labels = c("0", "4", "8"), guide = FALSE) +
  scale_linetype_manual(name = "Starting \nposition (m)", labels = c("0", "4", "8"), values = c("dotted", "dashed", "solid"), guide = guide_legend(reverse = TRUE)) +
   geom_curve(aes(x = -.05, y = 0, xend = -.005, yend = 0), arrow.fill = "black", size = 0.3, curvature = 0, arrow = arrow(type = "closed", length = unit(0.3,"cm"))) +
  geom_curve(aes(x = -.05, y = 4, xend = -.005, yend = 4), arrow.fill = "white", size = 0.3, curvature = 0, arrow = arrow(type = "closed", length = unit(0.3,"cm"))) +
  geom_curve(aes(x = -.05, y = 8, xend = -.005, yend = 8), arrow.fill = "darkgrey", size = 0.3, curvature = 0, arrow = arrow(type = "closed", length = unit(0.3,"cm"))) +
  ylab("z (m)") +
  xlab("x (m)") +
  ylim(0, 20) +
  #xlim(-.05, .2) +
  scale_x_continuous(breaks = c(0, .10), limits = c(-.05, .11), labels = scales::number_format(accuracy = 0.01)) +
  facet_wrap(~ radius, labeller = labeller(radius = radii.labs)) +
  theme_plot +
  theme(legend.position = c(0.80, .20), legend.key.height = unit(0, 'cm'), legend.key.width = unit(0.5, 'cm'), legend.key = element_blank(), legend.spacing.y = unit(0.1, "cm"), legend.margin = margin(0.05,0.05,0,0, unit = "cm"), legend.key.size = unit(0, 'cm'), plot.title = element_text(size = 19, face = "bold"), strip.text = element_text(size = 12, face = "bold"), axis.title.x = element_text(size = 12), axis.text.x = element_text(size = 12), axis.title.y = element_text(size = 12), axis.text.y = element_text(size = 12), title = element_text(size = 7), legend.title = element_text(size = 10), legend.text = element_text(size = 10))

"1500 radius curve"
radius.1500.plot <- ggplot() +
  geom_path(data = midline %>%
              dplyr::filter(radius == 1500), mapping = aes(x = x * -1, y = z), size = 1) +
  geom_path(data = onset.timecourse %>%
              dplyr::filter(pNum != 10, radius == 1500) %>%
              dplyr::group_by(startingpos, radius, frame) %>%
              dplyr::summarise(x = mean(rotatedX), z = mean(rotatedZ)), mapping = aes(x = x, y = z, linetype = as.factor(startingpos))) +
  geom_point(overall.dat %>%
               dplyr::filter(pNum != 10, radius == 1500) %>%
...
"2000 radius curve"
radius.2000.plot <- ggplot() +
  geom_path(data = midline %>%
              dplyr::filter(radius == 2000), mapping = aes(x = x * -1, y = z), size = 1) +
  geom_path(data = onset.timecourse %>%
              dplyr::filter(pNum != 10, radius == 2000) %>%
              dplyr::group_by(startingpos, radius, frame) %>%
              dplyr::summarise(x = mean(rotatedX), z = mean(rotatedZ)), mapping = aes(x = x, y = z, linetype = as.factor(startingpos))) +
  geom_point(overall.dat %>%
               dplyr::filter(pNum != 10, radius == 2000) %>%
...
g <- arrangeGrob(radius.1000.plot, radius.1500.plot, radius.2000.plot, nrow = 1)
  ggsave(here::here("TvA_curves_analysis_data/Plots/Fig9.tiff"), plot = g, width = 15, height = 10, units = 'cm', dpi = 600, type = 'cairo')
```
...
g <- arrangeGrob(accum.plot.rt, thresh.plot.rt, data.plot.rt, nrow = 1)
ggsave(here::here("TvA_curves_analysis_data/Plots/Fig10.tiff"), g,  width = 15, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 11: lateral position error results

```{r}
"steering bias means and confidence intervals"
...
g <- arrangeGrob(accum.plot.lpe, thresh.plot.lpe, data.plot.sb, nrow = 1)
ggsave(here::here("TvA_curves_analysis_data/Plots/Fig11.tiff"), g,  width = 15, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```
...
ggsave(here::here("TvA_curves_analysis_data/Plots/Fig12.tiff"), g,  width = 15, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Modelling for reaction time - Table 2
...
"Model fits for Gaussian, Gamma and Inverse Gaussian distribtuions"
...
stargazer(invgaus.rt.no.correlation, type = "text", 
          ...
          model.names = FALSE,
          title = "Fixed effects",
          out = here::here("TvA_curves_analysis_data/Plots/rt.table1.html")
          )
```
...
stargazer(gamma.sb.no.correlation, type = "text", 
          ...
          model.names = FALSE,
          title = "Fixed effects",
          out = here::here("TvA_curves_analysis_data/Plots/sb.table1.html")
)
```
...
stargazer(gamma.sr.no.correlation, type = "text", 
          ...
          model.names = FALSE,
          title = "Fixed effects",
          out = here::here("TvA_curves_analysis_data/Plots/sr.table1.html") 
          )
```