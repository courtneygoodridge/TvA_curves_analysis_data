...
"0 m starting position condition for each radius"
ggplot() +
  geom_path(as.data.frame(latency.list[1]) %>%
              dplyr::filter(timestamp <= 3), mapping = aes(x = timestamp, y = sb.diff), col = "red") +
  geom_path(as.data.frame(latency.list[4]) %>%
              dplyr::filter(timestamp <= 3), mapping = aes(x = timestamp, y = sb.diff), col = "yellow") +
  geom_path(as.data.frame(latency.list[7]) %>%
              dplyr::filter(timestamp <= 3), mapping = aes(x = timestamp, y = sb.diff), col = "black")

ggplot() +
  geom_path(as.data.frame(latency.list[1]) %>%
              dplyr::filter(timestamp <= 3), mapping = aes(x = timestamp, y = visangle.diff), col = "red") +
  geom_path(as.data.frame(latency.list[4]) %>%
              dplyr::filter(timestamp <= 3), mapping = aes(x = timestamp, y = visangle.diff), col = "yellow") +
  geom_path(as.data.frame(latency.list[7]) %>%
              dplyr::filter(timestamp <= 3), mapping = aes(x = timestamp, y = visangle.diff), col = "black")
