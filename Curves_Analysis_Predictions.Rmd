...
### Error accumulation when steering toward curves - Predictions
...
This script produces Threshold and Accumulator predictions. Visual angle is calculated between their current direction and a point 10 m ahead on the road-line through the curve. Visual angle is then accumulated and reaction times/lateral position errors are selected when the accumulated value surpasses a fixed "decision boundary". For the Threshold predictions, the non-accumulated visual angle is used and reaction times and lateral position errors are selected when it surpasses a fixed threshold value. 
