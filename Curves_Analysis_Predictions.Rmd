...
ggplot(latency.df %>%
         dplyr::filter(timestamp < 2.5), mapping = aes(x = timestamp, y = visangle.diff, col = as.factor(radius.x))) +
  geom_path() +
  facet_wrap(~ startingpos)
```
...
for (i in 1:9){
  ...
  for (a in c(.3)){
    ...
    latencies_tmp <- latencies_tmp %>%
      ...
      dplyr::mutate(accum = cumsum(visangle.diff)) %>%
