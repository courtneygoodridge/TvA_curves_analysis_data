...
ggplot(latency.df %>%
         dplyr::filter(startingpos == 0, timestamp <= 3.5), mapping = aes(x = timestamp, y = steeringbias, col = as.factor(radius.x))) +
  geom_line() +
  facet_wrap(~ startingpos)

ggplot(latency.df %>%
         dplyr::filter(startingpos == 0, timestamp <= 3.5), mapping = aes(x = timestamp, y = visangle, col = as.factor(radius.x))) +
  geom_line() +
  facet_wrap(~ startingpos)
```
